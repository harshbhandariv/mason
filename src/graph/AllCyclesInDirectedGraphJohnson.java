 ...
 *         Space complexity - O(E + V + s) where s is sum of length of all
 *         cycles.
 *
...
public class AllCyclesInDirectedGraphJohnson {
  ...
  public List<List<Vertex<Integer>>> simpleCyles(Graph<Integer> graph) {
    ...
    while (startIndex <= graph.getAllVertex().size()) {
      ...
      // this creates graph consisting of strongly connected components only and then
      // returns the
  ...
  private boolean findCyclesInSCG(
    ...
    // if cycle is found with current vertex then recursively unblock vertex and all
    // vertices which are dependent on this vertex.
    if (foundCycle) {
      // remove from blockedSet and then remove all the other vertices dependent on
      // this vertex from blockedSet
      unblock(currentVertex);
    } else {
      // if no cycle is found with current vertex then don't unblock it. But find all
      // its neighbors and add this
      // vertex to their blockedMap. If any of those neighbors ever get unblocked then
      // unblock current vertex as well.
      for (Edge<Integer> e : currentVertex.getEdges()) {
