...
public class TarjanStronglyConnectedComponent {
  ...
  private void sccUtil(Vertex<Integer> vertex) {
    ...
    for (Vertex<Integer> child : vertex.getAdjacentVertexes()) {
      ...
      } // if child is on stack then see if it was visited before vertex's low time. If
        // yes then update vertex's low time to that.
    ...
    // if vertex low time is same as visited time then this is start vertex for
    // strongly connected component.
    // keep popping vertices out of stack still you find current vertex. They are
    // all part of one strongly
    // connected component.
